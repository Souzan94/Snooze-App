...
function handleToggleSnoozeField(checkbox) {
    if (checkbox.checked) {
        createSnoozeField()
            .then(fieldId => {
                return addFieldToAllForms(fieldId) // Add the field to all forms
                    .then(() => fieldId); // Pass fieldId forward
            })
            .then(fieldId => {
                return saveSettings({
                    subdomain: subdomain,
            ...
            .catch(error => {
                console.error("Error setting up snooze field:", error);
            });
    } else {
        deleteSnoozeField()
            .then(() => {
...
function createSnoozeField() {
    return client.request({
        ...
        data: JSON.stringify({
            ticket_field: {
                type: 'text',
                title: 'Total Snoozed Time (seconds)',
...
function getSnoozeFieldValue(ticketId) {
    return client.request({
        ...
        const field = response.ticket.custom_fields.find(f => f.id === 28780385622034);
        return field ? parseInt(field.value, 10) || 0 : 0;
    });
}

function addFieldToAllForms(fieldId) {
    return client.request({
        url: '/api/v2/ticket_forms.json',
        type: 'GET',
        contentType: 'application/json'
    }).then(function(formsResponse) {
        const forms = formsResponse.ticket_forms || [];

        const updatePromises = forms.map(function(form) {
            if (!form.ticket_field_ids.includes(fieldId)) {
                form.ticket_field_ids.push(fieldId);
            }
            return client.request({
                url: `/api/v2/ticket_forms/${form.id}.json`,
                type: 'PUT',
                contentType: 'application/json',
                data: JSON.stringify({
                    ticket_form: {
        ...
        console.log(`Field ${fieldId} added to all forms successfully.`);
    }).catch(function(error) {
        console.error('Error adding field to forms:', error);
        throw error;
    });
}
