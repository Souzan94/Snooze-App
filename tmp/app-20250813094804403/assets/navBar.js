    ...
    client.get('currentUser').then(function(data) {
        ...
        client.context().then(function(context) {
            ...
            getSettings().then(settings => {
                if (settings && settings.isChecked) {
...
createTrigger().then(triggerId => {
            saveSettings(subdomain, triggerId, true)
        }).catch(error => {
   ...
   getSettings().then(settings => {
            if (settings) {
                deleteTrigger(settings.triggerId); 
                saveSettings(subdomain, settings.triggerId, false);
            } else {
            }
        }).catch(error => {
            console.error("Error getting settings:", error);
...
function saveSettings(subdomain, triggerId, isChecked) {
...
function getSettings() {
    ...
    return client.request({
        ...
        const responseBody = JSON.parse(response.body);
        if (response.statusCode === 200) {
            // Extract triggerId and isChecked from the response
            const triggerId = responseBody.triggerId;
            const isChecked = responseBody.isChecked;
            // Return an object containing triggerId and isChecked
            return {
                triggerId: triggerId,
                isChecked: isChecked
            };
