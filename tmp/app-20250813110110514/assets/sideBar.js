...
restartOption.addEventListener("click", function() {
    const secondsValue = convertToSeconds(unitSelectValue, timeInputValue);

    updateSnoozeField(ticketId, secondsValue)
        .then(() => {
            notifyALLUsers("restartTimer", ticketId);
        })
        .catch(error => {
            console.error("Error updating snooze field on restart:", error);
        });
});
...
function getSettings() {
    ...
    return client.request({
        ...
        if (response.statusCode === 200) {
            ...
            const {  snoozefieldId, isSnoozefieldChecked } = responseBody;
        ...
        console.error('Error get ting settings:', error);
        throw error;
    });
}
